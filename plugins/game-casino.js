...
let handler = async (m, { conn, args, usedPrefix, command, DevMode }) => {
  ...
  cooldowns[m.sender] = Date.now()
        ...
        if (args.length < 1) return conn.reply(m.chat, '🚩 Ingresa la cantidad de ' + `*💰 Pesetas*` + ' que deseas aportar contra' + ` *Kuroda*` + `\n\n` + '`Ejemplo:`\n' + `> *${usedPrefix + command}* 100`, m, rcanal)

        if (user.limit >= count * 1) {
            user.limit -= count * 1
            if (Aku > Kamu) {
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Kuroda* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *PERDISTE* ${formatNumber(count)} 💰 Pesetas.`.trim(), m, rcanal)
            } else if (Aku < Kamu) {
                user.limit += count * 2
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Kuroda* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username}, *GANASTE* ${formatNumber(count * 2)} 💰 Pesetas.`.trim(), m, rcanal)
            } else {
                user.limit += count * 1
                conn.reply(m.chat, '`🌵 Veamos que numeros tienen!`\n\n'+ `➠ *Kuroda* : ${Aku}\n➠ *${username}* : ${Kamu}\n\n> ${username} obtienes ${formatNumber(count * 1)} 💰 Pesetas.`.trim(), m, rcanal)
            }
        } else conn.reply(m.chat, `No tienes *${formatNumber(count)} 💰 Pesetas* para apostar!`.trim(), m, rcanal)
