...
let handler = async (m, {conn, usedPrefix}) => {
    ...
    let img = await (await fetch(`https://i.ibb.co/bdXFL4m/file.jpg`)).buffer()
    let txt = ` –  *S T A R S  -  U S E R*\n\n`
        txt += `┌  ✩  *🪪 Nombre* : ${user.name}\n`
        txt += `│  ✩  *💰 Pesetas* : ${toNum(user.limit)} ( *${user.limit}* )\n`
...
handler.help = ['Bal']
handler.tags = ['rpg']
handler.command = ['wallet', 'cartera', 'pesetas', 'bal', 'balance']
